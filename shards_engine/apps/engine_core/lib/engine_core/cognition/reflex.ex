defmodule EngineCore.Cognition.Reflex do
  @moduledoc "Tier 1: rat stimulus -> action table (spec 5.1)."
  ...
  defp has_loud_belief?(world, agent) do
    Enum.any?(agent.beliefs || %{}, fn {_place, abouts} ->
      Enum.any?(abouts || %{}, fn {_about, entry} ->
        entry.last_tick == world.tick and
          entry.last_fidelity >= 3 and
          entry.salience >= 6.0
      end)
    end)
  end
