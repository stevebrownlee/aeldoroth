defmodule ClientTUI.ChannelTest do
  ...
  wire — [join_ref, ref, event, topic, payload] — decoded into plain tuples.
  """
  ...
  test "encode produces the vsn 2.0.0 array envelope" do
    ...
    assert {:ok, [nil, "1", "declare_intent", "run:r1", %{"text" => "go east"}]} =
             Jason.decode(json)
  end

  test "decode routes phx_reply to a reply tuple with atom status" do
    wire = Jason.encode!([nil, "7", "phx_reply", "run:r1", %{"status" => "ok", "response" => %{"reply" => "T1"}}])

    assert {:ok, {:reply, "7", :ok, %{"reply" => "T1"}}} = Channel.decode(wire)
  end

  test "decode routes pushes with topic, event, payload" do
    wire = Jason.encode!([nil, nil, "perception", "run:r1", %{"text" => "You go east.", "tick" => 1}])
  ...
  test "decode joins the phx_reply for the initial join like any reply" do
    wire = Jason.encode!([nil, "1", "phx_reply", "run:r1", %{"status" => "ok", "response" => %{"state" => %{}}}])
